...
class ExecuteProcedure(BaseOperator):
...
class Processing(BaseOperator):
    ...
    def execute(self, context):
        ...
        df_t_cliente_cadastro = pd.DataFrame(pd.read_parquet(BytesIO(clientMinio.get_object(DATA_LAKE_VUON,past+'t_cliente_cadastro'+'.parquet').data)))
        df_t_cliente_gold  = pd.DataFrame(pd.read_parquet(BytesIO(clientMinio.get_object(DATA_LAKE_VUON,past+'t_cliente_gold'+'.parquet').data)))
        df_t_telefone = pd.DataFrame(pd.read_parquet(BytesIO(clientMinio.get_object(DATA_LAKE_VUON,past+'t_telefone'+'.parquet').data)))
    
        df_t_cliente_cadastro = df_t_cliente_cadastro[['dtainclusao', 'id_conta']]
        df_t_cliente_gold = df_t_cliente_gold[['id_pessoa_fila', 'edi_source', 'nomerazao', 'fantasia', 'email',\
                        'palavrachave', 'pais', 'uf', 'bairro', 'logradouro', 'cep', 'nrologradouro', 'cpf_cnpj',\
                        'digcgccpf', 'dtanascfund', 'origem', 'usuinclusao', 'sexo', 'cidade', 'fisicajuridica',\
                        'inscricaorg', 'id_conta', 'tipo_integracao', 'status_integracao']]
        df_t_telefone = df_t_telefone[['id_conta', 'fone_celular']]
        ...
        del df_t_cliente_cadastro
        del df_t_cliente_gold
        del df_telefone
        del df_t_telefone
        gc.collect()
        ...
        df['data_selecao'] = datetime.date(datetime.today())#[datetime.strptime(x, '%Y-%m-%d').date() for x in df['data_selecao']]
        ...
        df['id_pessoa_fila']= ExecuteProcedure.Get_Sequence(procedure="crm.p_sequence_vuon_integra_c5", p_num=parameters, tb= 'sequence')
        #print(len(teste))
